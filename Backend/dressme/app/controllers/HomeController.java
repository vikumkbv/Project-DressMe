...
public class HomeController extends Controller {
    ...
    public Result getColor(int gender,String color){

      //  if (color.equalsIgnoreCase("Caramel")||color.equalsIgnoreCase("Rajah")||color.equalsIgnoreCase("Crayola")||color.equalsIgnoreCase("Peach-Orange")||color.equalsIgnoreCase("Very Pale Orange")) {
if (color.matches("caucasoid")){
           if(gender==0){
            String [][] colors={{"Female","Caucasoid"},{"red","black","green","blue","purple"}};

            return ok(Json.toJson(colors));}
            if(gender==1){
                String [][] colors={{"Male","Caucasoid"},{"red","blue","black","light blue","orange"}};
                return ok(Json.toJson(colors));}
        }
        else if (color.equalsIgnoreCase("mongoloid")){
    if(gender==0){
        String [][] colors={{"Female","Mongoloid"},{"red","black","light blue","dark grey","light purple"}};
        return ok(Json.toJson(colors));}
    if(gender==1){
        String [][] colors={{"Male","Mongoloid"},{"black","white","blue","dark grey","dark red"}};
        return ok(Json.toJson(colors));}
}
else if(color.equalsIgnoreCase("negroid")) {
    if(gender==0){
        String [][] colors={{"Female","Negroid"},{"white","light grey","black","dark blue","dark grey"}};
        return ok(Json.toJson(colors));}
    if(gender==1){
        String [][] colors={{"Male","negroid"},{"white","black","green","dark blue","dark brown"}};
        return ok(Json.toJson(colors));}
}
else {
    return ok("The colors can be either mongoloid,caucasoid or negroid and the gender is 0 for female and 1 for male");
}
    ...
    public Result getColorNameFromRgb(int r, int g, int b) {
        ...
        if (closestMatch != null) {
            return ok(closestMatch.getName());
        } else {
            return ok("No matched color name.");
        }
    }
